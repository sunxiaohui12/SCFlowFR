...
class ShortcutFlowModelWOShortcutLoss(FlowModel):
    def __init__(
        self,
        segment_K: int = 128,
    ...
    def _get_mixed_targets(self, x1, x0, ema_model, **cond_kwargs):
        ...
        vt = vt_flow
        t = t_flow
        dt = torch.cat([torch.full((flow_size,), log2_sections, device=device)], dim=0)
...
class ShortcutFlowModelWOCondition(FlowModel):
    def __init__(
        self,
        segment_K: int = 128,
        boostrap_every: int = 4,
