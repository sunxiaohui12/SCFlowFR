# Borrowed from https://github.com/xinntao/BasicSR/blob/master/basicsr/utils/matlab_functions.py
...
@torch.no_grad()
def imresize(img, scale=None, size=None, antialiasing=True):
    ...
    if not scale:
        scale = size[0] * 1.0 / in_h
    out_h, out_w = math.ceil(in_h * scale), math.ceil(in_w * scale)
    if size:
        assert out_h == size[0] and out_w == size[1]
    kernel_width = 4
    kernel = "cubic"
