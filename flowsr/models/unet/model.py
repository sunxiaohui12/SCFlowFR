...
class EfficientShortcutUnet(EfficientUNet):
    """
    A variant of EfficientUNet for shortcut flow
    """
    def forward(self, x, t, dt, context=None, context_ca=None, **kwargs):
        if context_ca is not None:
            raise NotImplementedError("Cross-attn conditioning not supported yet.")
        
        # TODO: add support for kwargs (cross-attn conditioning)
        t_emb = timestep_embedding(t, self.model_channels)
        dt_emb = timestep_embedding(dt, self.model_channels)
        emb = self.time_embed(t_emb + dt_emb)
