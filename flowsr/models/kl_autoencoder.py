...
try:
    ...
    print(
        "[AutoencoderKL] Please update PyTorch to 2.0 or higher to use efficient attention."
    )
...
class MemoryEfficientAttnBlock(nn.Module):
    ...
    def __init__(self, in_channels, natten_kernel_size=-1, use_null_attention=False):
        ...
        if natten_kernel_size > -1:
            assert (
                NATTEN_IS_AVAILBLE
            ), "natten_kernel_size > -1 but natten is not available"
...
def make_attn(
    ...
    if attn_type == "vanilla":
        assert not (
            use_null_attention and natten_kernel_size > -1
        ), "use_null_attention and natten_kernel_size > -1 are mutually exclusive"
        ...
        return MemoryEfficientAttnBlock(
            in_channels,
            natten_kernel_size=natten_kernel_size,
            use_null_attention=use_null_attention,
        )
        ...
        **kwargs,
        ...
        if exists(ckpt_path):
            assert os.path.exists(
                ckpt_path
            ), f"[AutoencoderKL] Checkpoint {ckpt_path} not found!"
            ...
            warnings.warn(
                f"[AutoencoderKL] No checkpoint provided. Random initialization."
            )
